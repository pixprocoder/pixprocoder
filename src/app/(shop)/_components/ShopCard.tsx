...
const ShopCard = ({ item }: any) => {
  ...
  return (
    <Link
      href={`/shop/${item.id}`}
      className=" shadow-lg hover:shadow-purple-950 border border-purple-950 bg-black rounded-sm flex p-2  flex-col"
    >
      <div className="w-full h-40 overflow-hidden  flex justify-center items-center ">
        <img src={item.image} alt="image" />
      </div>
      <div className="">
        <p className="text-sm text-gray-200 mt-2 overflow-hidden text-ellipsis whitespace-nowrap w-full">
          {item.title}
        </p>
